...
class ChatBot:
    def __init__(
        ...
        if self.execution_context.value == ExecutionContext.LOCAL.value:
            ...
            embeddings = HuggingFaceEmbeddings(model_name=PREPARATION_MODEL_NAME)
            ...
            self.serving_mode = DATABRICKS_SERVING_MODE
        ...
        try:
            if self.execution_context.value == ExecutionContext.DATABRICKS.value:
                if (
                        ...
                        == DATABRICKS_SERVING_MODE.NOTEBOOK_HOSTED_API.value
                ):
                    logging.info(
                    ...
                    return Answer(question=question, answer=answer.strip().capitalize())
                        ...
                        self.serving_mode.value
                        == DATABRICKS_SERVING_MODE.NOTEBOOK_API.value
                ):
                    logging.info(
                ...
                similar_docs = self.get_similar_docs(question, similar_doc_count=int(SOURCE_DOCUMENTS_MAX_COUNT))
                ...
                return (
                    ...
                    else Answer.to_html(question=question, answer=answer.strip().capitalize())
                )
